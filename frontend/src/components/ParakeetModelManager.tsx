...
export function ParakeetModelManager({
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(`${displayInfo?.icon || '✓'} ${displayName} bereit!`, {
  ...
  const selectModel = async (modelName: string) => {
    ...
    const displayInfo = getModelDisplayInfo(modelName);
    const displayName = displayInfo?.friendlyName || modelName;
    toast.success(`Zu ${displayName} gewechselt`, {
      duration: 3000
    });
  };
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.success(`${displayName} gelöscht`, {
        description: 'Modell wurde entfernt, um Speicherplatz freizugeben',
        duration: 3000
      });
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          Verwendet {getModelDisplayName(selectedModel)} für Transkription
        </motion.div>
      )}
    </div>
  );
