...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError('Audio-Geräte konnten nicht geladen werden. Bitte überprüfe deine System-Audio-Einstellungen.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError('Keine Mikrofon-Geräte zum Überwachen gefunden');
        return;
      }
      ...
      setError('Audio-Pegelüberwachung konnte nicht gestartet werden');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">Audio-Geräte</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Keine Mikrofon-Geräte gefunden</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">Mikrofon-Pegel:</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder="System-Audio auswählen" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">Standard-System-Audio</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Keine System-Audio-Geräte gefunden</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>Mikrofon:</strong> Nimmt deine Stimme und Umgebungsgeräusche auf</p>
        <p>• <strong>System-Audio:</strong> Nimmt Computer-Audio auf (Musik, Anrufe usw.)</p>
        {isMonitoring && (
          <p>• <strong>Mikrofon-Pegel:</strong> Grün = gut, Gelb = laut, Rot = zu laut</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>Tipp:</strong> Klicke auf „Mikrofon testen“, um zu prüfen, ob dein Mikrofon funktioniert</p>
        )}
      </div>
    </div>
  );
