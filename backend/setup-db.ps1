...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.kernsatz\meeting_minutes.db",
        "$env:USERPROFILE\Documents\kernsatz\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/kernsatz-backend/*/backend/meeting_minutes.db"
    }
